...
def optimizar():  
    ...
    print("Costo total anual: $", value(modelo.objective))
    ...
    for i in ['A','B','C']:
        for j in ['B','C','D','E']:
            if value(X_ij[i][j]) > 0:
                print(f"X_{i}{j}: {value(X_ij[i][j]):.2f} ton/año")
    
    print("\nFlujo de alúmina (Y_jk):")
    for j in ['B','C','D','E']:
        for k in ['D','E']:
            if value(Y_jk[j][k]) > 0:
                print(f"Y_{j}{k}: {value(Y_jk[j][k]):.2f} ton/año")

    print("\nPlantas abiertas (W_j):")
    for j in ['B','C','D','E']:
        print(f"W_{j}: {value(W_j[j])}")
